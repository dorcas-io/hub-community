            ...
            <div class="col s12 m4">
                <div class="card">
                    ...
                    <div class="card-content">
                        <span class="card-title activator grey-text text-darken-3">@{{ team.name }}<i class="material-icons right">edit</i></span>
                        ...
                        <form method="post" action="" v-on:submit.prevent="updateDetails">
                            ...
                            <div class="row">
                                <div class="input-field col s12">
                                    <input id="name" type="text" v-model="team.name" maxlength="80" required>
                                    <label for="name">Team Name</label>
                                </div>
                                <div class="input-field col s12">
                                    <textarea id="description" class="materialize-textarea" v-model="team.description">@{{ team.description }}</textarea>
            ...
            <div class="col s12 m8" v-on:click="clickAction">
                @if (!empty($team->employees['data']))
                    <table class="bootstrap-table responsive-table"
                        ...
                        <tbody>
                        @foreach ($team->employees['data'] as $employee)
                            <tr>
                                <td>
                    ...
                    <div class="col s12">
                        @component('layouts.slots.empty-fullpage')
                            @slot('icon')
                                group
                            @endslot
                            You can add add members to this team from your employees.
                            @slot('buttons')
        ...
        new Vue({
            ...
            methods: {
                updateDetails: function () {
                    ...
                    axios.put("/xhr/business/teams/" + context.team.id, {
                ...
                removeEmployee: function (attributes) {
                    ...
                    swal({
                        ...
                        axios.delete("/xhr/business/teams/" + context.team.id + "/employees", {
                            data: {employees: [id]}
                        }).then(function (response) {
                            console.log(response);
                            window.location = '{{ route('business.teams.single', [$team->id]) }}';
                        })
                            .catch(function (error) {
                                var message = '';
                                console.log(error);
